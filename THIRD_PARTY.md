...
`v2.0.0` 的 boot 和 rootfs 以 Debian 12 归档为基础。Debian 12 原始 rootfs 缺少完整 MPSS 文件，因此从已验证的 Debian 11 rootfs 复用了 `mba.mbn`、`modem.mdt` 和 `modem.b*` 文件集。GPT 与启动链文件经逐文件比较，与 `v1.0.1` 使用的文件一致。
